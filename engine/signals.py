...
def _get_db() -> sqlite3.Connection:
    ...
    # Create tables (without flushed index — added after migration)
    conn.executescript("""
    ...
    """)
    # Migrate: add flushed column if it doesn't exist yet
    cols = {r[1] for r in conn.execute("PRAGMA table_info(signals)").fetchall()}
    if "flushed" not in cols:
        conn.execute("ALTER TABLE signals ADD COLUMN flushed INTEGER DEFAULT 0")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_signals_flushed ON signals(flushed)"
    )
    conn.commit()
    return conn
...
def get_pending_signals() -> list[dict]:
    """Return signals recorded but not yet flushed/synced to the server."""
    try:
        conn = _get_db()
        rows = conn.execute(
            "SELECT id, skill_id, skill_name, session_id, kind, "
            "rework_count, score, created_at FROM signals "
            "WHERE flushed=0 ORDER BY created_at ASC LIMIT 200"
        ).fetchall()
        conn.close()
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=8) as resp:
            if resp.status == 200:
                ...
                conn.execute(
                    f"UPDATE signals SET flushed=1 WHERE id IN ({','.join('?'*len(ids))})",
                    ids
                )
                conn.commit()
                conn.close()

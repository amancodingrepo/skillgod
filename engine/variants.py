...
def submit_to_server(skill_name: str, skill_body: str,
                     category: str, confidence: float) -> None:
    """Fire-and-forget: push skill to the Railway server promotion queue."""
    import os as _os
    api = _os.environ.get("SKILLGOD_API", "")
    if not api:
        return
    try:
        import requests as _req
        from engine.license import get_cached_key as _key
    except ImportError:
        return
    try:
        _req.post(
            f"{api}/v1/vault/submit-skill",
            json={"name": skill_name, "body": skill_body,
                  "category": category, "confidence": confidence},
            headers={"X-License-Key": _key() or ""},
            timeout=5,
        )
    except Exception:
        pass
...
def auto_enqueue_candidates() -> int:
    ...
    for c in candidates:
        ...
        if row_id:
            added += 1
            submit_to_server(c["name"], c["content"], "meta", c["confidence"])
    return added

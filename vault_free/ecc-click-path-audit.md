...
description: Use when "Trace every user-facing button/touchpoint through its full state change sequence to find bugs where functions individually work but cancel each other out, produce wrong final state, or leave the UI in an inconsistent state. Use when: systematic debugging found no bugs but users report broken buttons, or after any major refactor touching shared state stores."
tags: [debug, refactor, function, click, audit]
triggers: [click, path, audit, state]
confidence: 0.85
origin_url: https://github.com/affaan-m/everything-claude-code
uses: 0
---
